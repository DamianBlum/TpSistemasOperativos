...
void actualizar_pcb(t_list *paquetes, t_PCB *pcb, t_log *logger)
{
    ...
    pcb->BX = (uint8_t)list_get(paquetes, 3);
    pcb->CX = (uint8_t)list_get(paquetes, 4);
    pcb->DX = (uint8_t)list_get(paquetes, 5);
    pcb->EAX = (uint32_t)list_get(paquetes, 6);
    pcb->EBX = (uint32_t)list_get(paquetes, 7);
    pcb->ECX = (uint32_t)list_get(paquetes, 8);
    pcb->EDX = (uint32_t)list_get(paquetes, 9);
